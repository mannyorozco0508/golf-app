...
// ============================================================================
// calculateMatchEngine MOVED OUT — see match-engine.js (v218)
// ============================================================================
// It lived here, and index.html and stats.html each carried an inline copy that
// SHADOWED it, because an inline <script> parses after the external ones. The two
// page copies had drifted (escapeHtml inside finalResult, a dead pressesByHole, a
// dropped t1Players/t2Players), so three different functions decided who pays
// whom depending on which screen a golfer was looking at.
//
// stats.html does not load this file, so the only way to give it the canonical
// engine was to give the engine a file of its own - exactly what handicap.js did
// for the relative-handicap helpers when Batch 3 hit the same wall.
//
// NOTHING ELSE IN THIS FILE CHANGED. computeRoundMoneyByPlayer (below) and
// buildLiveMatchState still call calculateMatchEngine as a plain global, the way
// they always did; match-engine.js must therefore be loaded BEFORE this file's
// callers run, which every page's <script> order now does and
// match_engine_load_order_test.js pins per page.
// ============================================================================
