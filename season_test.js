...
// season.html's REAL script tags, in its order. match-engine.js is in the list
// because the page loads it and because settlement-engine.js reaches
// calculateMatchEngine through a typeof guard: without it a MATCH round totals
// ZERO here and reports nothing. MODULE_PREREQS does not help - it applies to the
// file being loaded (season.js), not to the dependencies listed beside it.
const ENGINES = ['handicap.js', 'text-safe.js', 'action-model.js', 'match-engine.js', 'money-engine.js', 'aloha-bet.js', 'settlement-engine.js', 'pool-engine.js'];
...
// A MATCH round. EVERY OTHER FIXTURE IN THIS FILE IS gameFormat 'stroke', so none
// of them touches calculateMatchEngine at all - which is why the season tests were
// green while season.html was missing the match-engine.js tag entirely and would
// have totalled match rounds at zero on a real device.
function matchRound() {
    const players = [
        { id: 101, name: 'Marty', hcp: '0', team: 'Team 1', playingForMoney: true },
        { id: 102, name: 'Dee', hcp: '0', team: 'Team 2', playingForMoney: true }
    ];
    return {
        eventName: 'Cup Monday', courseName: 'Caledonia', gameFormat: 'match',
...
describe('THE FACTS ARE THE RESULTS ENGINES', () => {
    ...
    test('A MATCH ROUND reports its match money, not zero', () => {
        // THE GUARD FOR THE DEFECT THIS MERGE ALMOST SHIPPED. season.js totals a
        // round through computeCombinedNetTotals, which calls calculateMatchEngine
        // through a typeof guard - so a page or a realm without the engine returns
        // no match money AND NO ERROR. Marty wins all three holes of a $20 gross
        // match, so anything other than a non-zero, zero-sum pair means the engine
        // was not there.
        const data = matchRound();
        const facts = S.seasonRoundFacts(data);
        const combined = S.computeCombinedNetTotals(data, data.courseData, data.scores);
        assert.equal(facts.final, true, 'three of three holes scored is a finished round');
        assert.ok(facts.moneyByNorm.marty > 0,
            'Marty won every hole of a $20 match and the season totalled '
            + facts.moneyByNorm.marty + ' - the match engine is missing');
        assert.equal(facts.moneyByNorm.marty, combined.netByName.marty.net,
            'the season must report exactly what the round settles');
