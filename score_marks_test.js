...
describe('PRESENTATION', () => {
    ...
    test('birdie is one red ring and eagle is two, on both pages', () => {
        [idx, st].forEach(src => {
            ...
            rings.forEach(r => {
                const reds = (r.match(/--accent-red|#c1121f/g) || []).length;
                assert.equal(reds, 2, 'eagle must be a DOUBLE red ring');
                assert.ok(!/--brand-green|#0f4c3a/.test(r),
                    'the old green eagle ring must not linger');
            });
        });
    });
