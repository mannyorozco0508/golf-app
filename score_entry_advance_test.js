...
// THE RE-FIRING STUB. Wraps the harness stub's ref(): every value listener on
// events/<CODE> is remembered; set() on a score path writes into a shadow copy
// of the round and invokes that listener SYNCHRONOUSLY, as the real SDK does.
// window.__remote(patch) is the stub's own door for "another device's snapshot":
// it merges a scores patch and fires the listener without any local write.
// Event tracing (capture phase, score boxes only) records the order.
const REFIRE = `
(function () {
