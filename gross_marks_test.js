...
describe('MARKS ON THE PRIMARY SCORE ARE GROSS-ONLY', () => {
    ...
    test('score-marks.js still owns the gross birdie / eagle rings', () => {
        ...
        // v192: the wrapper also carries score-gap (score-gaps.js) after the mark class; the mark is still first and still scoreMarkClass's.
        assert.match(RS_CODE, /class="score-input-wrapper\$\{markClass\}\$\{gapBoxes\.has\(String\(p\.id\) \+ '_' \+ h\.hole\) \? ' score-gap' : ''\}"/);
...
describe('SERVICE WORKER', () => {
    ...
    test('CACHE_VERSION moved to v10', () => {
        assert.match(sw, /const CACHE_VERSION = 'golfapp-v228-admin-dialogs';/);
    ...
    test('the shell file list did NOT change', () => {
        ...
        // anything at all.
        assert.equal(entries.length, 56);  // 53: match-engine.js joined (v218: the one calculateMatchEngine; every match page loads it and every caller typeof-guards it, so a shell without it settles no match money and says nothing)  // 55: season.html and season.js joined (v222: the season ledger, landed from PR #17 - the branch was cut from v217 and said v218, which was taken)  // 52: attendance.js joined (v217)  // 51: aloha-bet.js joined (v215)  // 50: handicap-labels.js joined (v212: the "Index 18 · Course 17" label, loaded unguarded by four pages);  // 46: email-link-auth.js joined (v203: lobby email-link sign-in, keeps the anonymous uid);  // 45: score-gaps.js joined (v192: the missing-hole builder; index, leaderboard, settlement);  // 44: game.html joined (v186: the Game tab took Stats' nav slot; stats.html stays as the parity surface); 43: qrcode.min.js joined (2026-09-18: the Tournament QR library, vendored - it was a runtime CDN script; TOURNAMENT_SHELL only); 42: course-import-rules.js joined (v168: the pure import rules, lifted out of admin.html; tournament.html loads it too); 41: scorecard-rows.js joined (v163: settlement.html's Full Scorecard draws its rows from it, unguarded; leaderboard.html next); 40: organizer-gate.js joined (Wave 3, v147); 39: auth-boot.js joined (v139, every Consumer page); 38: live-skins.js joined (index, leaderboard, settlement); 37: firebase-auth-compat.js (tournament.html only)
