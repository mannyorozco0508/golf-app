...
describe('MARKS ON THE PRIMARY SCORE ARE GROSS-ONLY', () => {
    ...
    test('THE EXISTING GROSS EAGLE PRESENTATION IS PRESERVED', () => {
        // Deliberately NOT converted to a double circle in this batch.
        assert.match(SRC, /\.score-input-wrapper\.mark-eagle[^{]*\{[^}]*var\(--brand-green\)/,
            'the green eagle ring must survive');
...
describe('SERVICE WORKER', () => {
    ...
    test('CACHE_VERSION moved to v10', () => {
        assert.match(sw, /const CACHE_VERSION = 'golfapp-v10-gross-marks';/);
        assert.ok(!/const CACHE_VERSION = 'golfapp-v9-auto-dot-context';/.test(sw));
    });
