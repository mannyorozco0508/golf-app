...
// ---------------------------------------------------------------------------
describe('CLUB ROUND IS A PRESET, NOT A PRODUCT', () => {
    test('the tile no longer calls itself an Event or wears a trophy', () => {
        const tile = ADMIN.slice(ADMIN.indexOf('id="hw-club"') - 400, ADMIN.indexOf('id="hw-quick"'));
        assert.match(tile, /Club Round/);
        assert.ok(!/Club Event<\/div>/.test(tile), 'the old label read as a third category');
        assert.ok(!/\u{1F3C6}/u.test(tile), 'a trophy reads as tournament administration');
    });

    test('the tile says outright that it is the same round', () => {
        assert.match(ADMIN, /Same round, set up for club formats/);
        assert.match(ADMIN, /the same round as Quick Round, with club formats/,
            'the framing line must state the relationship, not just hint at it');
    });

    test('it creates the same round Quick Round creates', () => {
...
describe('THE BRANDING SEAM IS SMALL AND THE TWO PRODUCTS ARE INDEPENDENT', () => {
    ...
    test('page titles describe function, not brand, so a rename touches few files', () => {
        [['index.html', 'Live Scorecard'], ['leaderboard.html', 'Live Leaderboard'],
         ['settlement.html', 'Round Settlement'], ['trip.html', 'Trip']].forEach(([f, t]) => {
