...
describe('THE CACHES MOVED', () => {
    test('the caches moved for this wave and have not moved back; both pages stay in sw.js SHELL_FILES', () => {
        // Later waves move both keys on (v45 / v170: hero lockup). The Moved-to
        // notes for the dark-mode wave stay in both files either way.
        assert.match(read('build-shell.js'), /Moved to v44\. Dark mode is gone from both tournament pages/);
        assert.match(read('sw.js'), /Moved to v169: dark mode is gone from the Tournament product/);
        const t = /cacheName: 'tournament-v(\d+)-/.exec(read('build-shell.js'));
        const c = /const CACHE_VERSION = 'golfapp-v(\d+)-/.exec(read('sw.js'));
        assert.ok(t && Number(t[1]) >= 44, 'tournament key at or past v44: ' + (t && t[0]));
        assert.ok(c && Number(c[1]) >= 169, 'consumer key at or past v169: ' + (c && c[0]));
