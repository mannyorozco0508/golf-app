...
describe('CONSUMER — a golfer\u2019s app, not an organizer\u2019s', () => {
    ...
    test('1b. Consumer is exactly two modes: Game Day and Road Trip', () => {
        // The product boundary, asserted positively. Club Round moved to the other
        // product; removing a tile must not quietly remove its neighbours either.
        const admin = read('admin.html');
        ['hw-quick', 'hw-trip'].forEach(id =>
            assert.match(admin, new RegExp('id="' + id + '"'), id + ' must remain a Consumer mode'));
        assert.ok(!/id="hw-club"/.test(admin), 'Club Round belongs to the Club/Tournament product');
    ...
    test('2. the Tournament ROUTE is preserved even though Consumer stopped advertising it', () => {
        // The promo card was removed at the product separation. The helpers behind it
        // were NOT: selectHomeWidget('tournament') still routes through them for an
        // installed PWA holding cached older markup, and the Club/Tournament app reuses
        // them. Deleting them would be destroying the second product, not separating it.
        const admin = read('admin.html');
        assert.ok(!/cross-product-exit">/.test(admin), 'the promo card is gone from Consumer');
        assert.match(admin, /function openTournamentsApp/, 'the route helper must survive');
        assert.match(read('product-links.js'), /function tournamentUrl/,
            'the shared origin helper must survive - the Club app reuses it');
        assert.match(admin, /if \(type === 'tournament'\) \{ openTournamentsApp\(\); return; \}/,
            'the cached-markup compatibility branch must survive');
    });

    test('2b. the retired promo markup does not come back', () => {
        const admin = read('admin.html');
        assert.ok(!/id="open-tournaments-link"/.test(admin),
            'the promo link was retired at the product separation');
        assert.ok(!/Running a tournament or charity event\?/.test(admin),
            'Consumer no longer advertises the other product');
    });
    ...
    test('12b. Road Trip no longer surfaces Tournament management', () => {
        const trip = read('trip.html');
        assert.ok(!/Manage in Tournaments/.test(trip), 'the button is gone from Consumer');
        assert.ok(!/Tournaments in This Trip/.test(trip), 'the list section is gone from Consumer');
        assert.ok(!/Create a Tournament for This Trip/.test(trip),
            'Consumer never created tournaments and must not start claiming to');
    });

    test('12c. THE TRIP-TOURNAMENT DATA RELATIONSHIP SURVIVED THE UI REMOVAL', () => {
        // The single most important assertion in this batch. Removing the UI must not
        // remove the schema, the read path, or the outbound helper - the Club/Tournament
        // app reads trips/<code>/tournaments to find its trip context. If a future
        // cleanup deletes these, that app loses the relationship silently.
        const trip = read('trip.html');
        ...
        assert.match(trip, /function renderTournamentsList/,
            'the render function is retained for reuse by the Club app');
