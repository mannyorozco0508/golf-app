...
describe('SERVICE WORKER - install and update behaviour', () => {
    ...
    test('the cache version is a bumped golfapp key, and moves whenever the shell changes', () => {
        ...
        assert.equal(CACHE_NAME, 'golfapp-v13-no-date-gate', 'Cache key changed - if that was deliberate, update this line; every installed device drops its old cache on activate.');
    });
});
