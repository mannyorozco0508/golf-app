...
// ---------------------------------------------------------------------------
// THE CALENDAR-DATE SAFETY INVARIANT — ALL SEVEN PAGES
// ---------------------------------------------------------------------------
//
// WHY THIS EXISTS. The block above pinned the beta gate on index.html only. Six
// other pages carried the identical time bomb - admin, leaderboard, settlement,
// stats, skins and sidematches - and the guard was blind to every one of them, so
// when the date rolled over the app went dark everywhere except the scorecard.
// Two of those pages were worse than a hardcoded date: they read a global
// expiration value out of Firebase, so a single database write could black out
// every installed copy of the app at once.
//
// THE INVARIANT: no supported app page may wipe document.body, throw/abort,
// redirect away, or otherwise block normal functionality solely because the
// current date is past a beta/end/expiration date.
//
// This is deliberately written against a PAGE LIST rather than one file, so a new
// page cannot quietly reintroduce the mechanism.

describe('CALENDAR-DATE SAFETY — no page may go dark because a date passed', () => {

    const PAGES = ['index.html', 'admin.html', 'leaderboard.html', 'settlement.html',
                   'stats.html', 'skins.html', 'sidematches.html'];

    // Comments describing the removed mechanism are not the mechanism. Strip them
    // first so a warning note cannot fail the guard, and so a real gate cannot hide
    // behind one.
    ...
    PAGES.forEach(page => {
        ...
        test(page + ' — the beta shutdown screen is gone', () => {
            const code = codeOf(read(page));
            assert.ok(!/Beta Period Complete/.test(code), page + ' still renders the shutdown screen');
            assert.ok(!/beta timeline has ended/.test(code), page + ' still renders the shutdown copy');
        });

        test(page + ' — startup is never aborted by a date', () => {
            const code = codeOf(read(page));
            assert.ok(!/Beta period expired/.test(code), page + ' still throws when the date passes');
        });

        test(page + ' — no date comparison wipes the page or redirects', () => {
            const code = codeOf(read(page));
            // Any place the page compares "now" against something, check that the
            // branch does not destroy the page.
            const DESTRUCTIVE = /(document\.body\.innerHTML\s*=|document\.documentElement\.innerHTML\s*=|location\.(replace|assign|href\s*=))/;
            const lines = code.split('\n');
            lines.forEach((line, i) => {
                if (!/new Date\(\)\s*[<>]/.test(line)) return;
                const window = lines.slice(i, i + 12).join('\n');
                assert.ok(!DESTRUCTIVE.test(window),
                    page + ' line ' + (i + 1) + ': a date comparison leads to a destructive action:\n' + window);
                assert.ok(!/throw /.test(window),
                    page + ' line ' + (i + 1) + ': a date comparison leads to a throw:\n' + window);
            });
        });

        test(page + ' — no remote expiration value is read', () => {
            const code = codeOf(read(page));
            assert.ok(!/beta_expiration/.test(code),
                page + ' still reads a Firebase expiration value - one write could black out every device');
            assert.ok(!/app_settings\/[a-z_]*expir/i.test(code));
        });
    });

    test('NO PAGE anywhere in the repo carries an equivalent gate under another name', () => {
        // Catch the same mechanism hiding behind different wording.
        const htmlFiles = fs.readdirSync(REPO_ROOT).filter(f => f.endsWith('.html'));
        const SUSPECT = /(END_DATE|EXPIR|KILL_SWITCH|SHUTDOWN|LOCKOUT|TRIAL_END|SUNSET)/i;
        htmlFiles.forEach(f => {
            const code = codeOf(read(f));
            code.split('\n').forEach((line, i) => {
                if (!SUSPECT.test(line)) return;
                const window = code.split('\n').slice(i, i + 12).join('\n');
                assert.ok(!/document\.body\.innerHTML\s*=/.test(window),
                    f + ' line ' + (i + 1) + ' looks like a disguised kill switch:\n' + window);
    ...
    test('the Master Beta Switch UI and its write path are gone', () => {
        const admin = read('admin.html');
        assert.ok(!/Master Beta Switch/.test(codeOf(admin)), 'the panel heading survives');
        assert.ok(!/updateBetaExpiration/.test(admin), 'the Firebase writer survives');
        assert.ok(!/secret-exp-date/.test(admin), 'the date input survives');
        assert.ok(!/beta_expiration/.test(admin), 'a beta_expiration reference survives');
    });

    test('the six repaired pages still load their real content', () => {
        // A gate removed by deleting the page would also pass the tests above.
        [['admin.html', 'wizard-step-2'], ['leaderboard.html', 'leaderboard'],
         ['settlement.html', 'receipt-scorecard'], ['stats.html', 'stats-content'],
         ['skins.html', 'birdie'], ['sidematches.html', 'confirmAutoPairCreate']]
            .forEach(([f, marker]) => {
                assert.ok(read(f).includes(marker), f + ' lost its page content');
            });
    });
});
