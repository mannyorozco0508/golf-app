...
describe('ZERO-SUM AUDIT — every player-vs-player monetary game', () => {
    ...
    test('DOCUMENTED FINDING (not a test-writing bug): Dots/Junk is NOT zero-sum — a dot-earner\'s winnings are not debited from anyone else', () => {
        const cd = makeCourseData(9);
        const players = makePlayers(['A', 'B'], [0, 0]);
        const dots = { h3: { [`p${players[0].id}`]: ['birdie', 'greenie'] } }; // only A earns dots, B earns none
        const scores = {};
        const result = engine.computeRoundMoneyByPlayer({ players, gameFormat: 'dots', dots, dotPointVal: 3 }, cd, scores);
        const sum = result.players.reduce((s, p) => s + p.net, 0);
        // This is the CONFIRMED, real, current behavior — not what a zero-sum game would produce.
        // Documenting it precisely rather than asserting a false zero-sum invariant, and rather
        // than silently inventing a redistribution rule that isn't in the current product.
        assert.equal(sum, 6, 'A\'s +$6 in dots is not sourced from B — this game currently pays out independently per player, not from a shared pot');
        assert.equal(result.players.find(p => p.name === 'B').net, 0);
    });

    test('DOCUMENTED FINDING: Birdie Pool has the same non-zero-sum characteristic as Dots', () => {
        const cd = makeCourseData(9);
        const players = makePlayers(['A', 'B'], [0, 0]);
        const scores = {};
        ...
        assert.equal(sum, 5, 'confirms the identical pattern to Dots — a birdie\'s payout is not debited from the rest of the field');
    });

    test('DOCUMENTED FINDING: the real, user-facing consequence — simplifyDebts produces ZERO transactions for a non-zero-sum contribution, even though Final Results would show real money owed', () => {
        // This is the concrete, visible problem: if Dots or Birdie Pool money isn't offset by
        // another zero-sum game in the same round, "Final Results" can show a player with a
        // positive balance while "Who Pays Who" shows nobody paying them anything at all — a
        // direct, confusing contradiction a golfer would actually see. Not fixed this batch —
        // fixing it would mean either inventing a redistribution rule for Dots/Birdie Pool
        // (new money math, explicitly out of scope) or restructuring how non-zero-sum pools
        // combine with zero-sum games, which is a real product decision, not a bug to patch
        // silently. Documented precisely here so it's a known, tracked finding, not a surprise.
        const netTotals = { A: 6, B: 0 };
        const transactions = engine.simplifyDebts(netTotals);
        assert.equal(transactions.length, 0, 'confirms: no one is asked to pay A their $6, even though Final Results would show it as won');
    });
